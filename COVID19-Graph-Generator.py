...
path = '/content/NotoSansCJKkr-Black.otf'
path2 = '/content/NotoSansCJKkr-Bold.otf'
path3 = '/content/NotoSansCJKkr-Medium.otf'
path4 = '/content/NotoSansCJKkr-Regular.otf'
...
def GenPlot(DataIn):
    ...
    left = dt.date(2020, 1,19)
    right = dt.date.today()
    tick_spacing = 5000
    days = mdates.drange(left,right,dt.timedelta(days=1))
    
    fig, [ax,timeline] = plt.subplots(nrows=2, ncols=1, sharex='col', gridspec_kw={'height_ratios': [5, 2]},figsize=(48,36),constrained_layout=True)
    ...
    ax2.grid(color='#ffffff', linestyle='-', linewidth=2, alpha=0.5)
...
#annotate line
    ...
    timeline.grid(True,'minor', 'x', color="#ffffff", alpha=0.5)
...
ServiceKey = u'ZCeTk3mu+xiUvh86KF8AtSJrhxGPEMSnsJORXMElUrOsCmwxPTra+c6SqnV/yQ1FAfUtkE+mcwIkS0TXeGc0JA=='
class req:
...
now = dt.datetime.now()
sp = np.datetime_as_string(np.arange('2020-03-11',(now+dt.timedelta(days=1)).strftime("%Y-%m-%d"),dtype='datetime64[D]'),unit='D')
sp = np.flip(sp)
df['stateDt'] = sp
df = df.iloc[::-1]
...
DateIn = sp
GenPlot(df[key_colums])
print(sp.tolist())
print(df[key_colums])
print(Confirm)
print("날짜\t\t확진자\t\t사망자\t격리해제\tSeq\n")
